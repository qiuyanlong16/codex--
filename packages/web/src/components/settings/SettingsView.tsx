...
export function SettingsView({
  ...
  const removeProvider = async (providerName: string) => {
    ...
    const others = settings.providers.filter(
      (item) => item.configured && item.name !== providerName,
    );
    if (others.length === 0) {
      setError(t("settings.byok.removeLastProvider"));
      return;
    }
    const fallback = others[0]!;
    const confirmed = window.confirm(
      t("settings.byok.removeConfirm", {
        provider: provider.label,
        fallback: fallback.label,
        defaultValue: `Remove ${provider.label}? The active model will fall back to ${fallback.label}.`,
      }),
    );
    if (!confirmed) return;
    setProviderSaving(providerName);
    try {
      const payload = await removeProviderWithFallback(token, settings, providerName);
      applyPayload(payload);
      if (payload.requires_restart) {
        setPendingRestartSections((prev) => ({ ...prev, image: true, runtime: true }));
      }
      await maybeRestartHostEngine(payload);
      ...
      <main className="min-w-0 flex-1 overflow-y-auto [scrollbar-gutter:stable]">
        <div
          className={cn(
            ...
            hostChromeInset && "pt-4 sm:pt-4 lg:pt-4",
          )}
        >
          <div className="mb-7">
            {!showSidebar ? (
  ...
  return (
    <aside
      className={cn(
        ...
        hostChromeInset ? "pt-4 md:pt-4" : "pt-4 md:pt-4",
      )}
    >
      <button
        type="button"
            ...
            <div className="flex items-center justify-end gap-2">
              {provider.configured ? (
                ...
                >
                  ...
                  {tx("settings.byok.removeProvider", "Remove")}
                </Button>
              ) : null}
              <Button
                size="sm"
